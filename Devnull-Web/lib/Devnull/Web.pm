...
#=============================================================================
# Web-intiated challenge ignoring. This interface was added few days before
# the tournament start, so it's one terrible kludge.
#=============================================================================

get '/chalignore' => sub {

  my $name = session('logname');
  my $challfile = setting('app')->{'challenges'};
  my $chalignore = setting('app')->{'chalignore'};

  #--- not logged in

  if(!$name) {
    return template 'challenge_ignore', { title => 'Disable Challenges' };
  }

  #--- get list of all challenges

  my @challenge_list = @{ setting('app')->{'challist'} };

  #--- read the challenge status file, and remove the challenges with 'success'
  #--- and 'ignore' status from the list of eligible challenges

  if($challfile) {
    open(my $f_chal, '<', $challfile);
    while (<$f_chal>) {
      chomp;
      my @parsed = split(/:/);

      # ignore other players
      next if $parsed[2] ne $name;

      # challenges with 'success' or 'ignore' cannot be disabled, remove them
      if($parsed[3] eq 'success' || $parsed[3] eq 'ignore') {
        @challenge_list = grep { $_ ne $parsed[1]; } @challenge_list;
        next;
      }
  ...
  #--- remove already ignored, but unprocessed challenges

  my @challenge_list_copy = @challenge_list;

  for my $chal (@challenge_list_copy) {
    if(
      -f sprintf(
        '%s/%s-%s-ignore', $chalignore, $chal, $name
      )
    ) {
      @challenge_list = grep { $_ ne $chal; } @challenge_list;
      next;
    }
  }
  ...
  template 'challenge_ignore', {
    title => 'Disable Challenges',
    logname => $name,
    challfile => $challfile,
    chlist => \@challenge_list,
  };

};

get '/chalignore/:challenge' => sub {

  my $chalignore = setting('app')->{'chalignore'};
  my $name = session('logname');
  my $challenge = route_parameters->get('challenge');

  #--- check validity of the request

  my $challenge_list = setting('app')->{'challist'};
  if(!(grep { $_ eq $challenge } @$challenge_list)) {
    redirect '/chalignore';
  }
  ...
  if($name) {
    touch(
      sprintf(
        '%s/%s-%s-ignore', $chalignore, $challenge, $name
      )
    );
  }
